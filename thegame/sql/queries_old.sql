...
INSERT INTO characters (account_id, name, class_id, current_room_id) VALUES (?, ?, ?, 1);

-- name: update_character_stats!
UPDATE characters SET 
    level = ?, experience = ?, gold = ?, rage_current = ?, rage_max = ?,
    hit_points_current = ?, hit_points_max = ?, attack = ?, total_power = ?,
...
-- name: equip_item!
INSERT OR REPLACE INTO character_equipment (character_id, slot_id, item_id) VALUES (?, ?, ?);
...
FROM characters c
JOIN character_classes cc ON c.class_id = cc.id
WHERE c.current_room_id = ? AND c.hit_points_current > 0
ORDER BY c.total_power DESC;

-- name: create_crew!
INSERT INTO crews (name, leader_id, description) VALUES (?, ?, ?);
...
-- name: join_crew!
INSERT INTO crew_members (crew_id, character_id, role) VALUES (?, ?, ?);
...
INSERT INTO crew_vault (crew_id, item_id, quantity, deposited_by) VALUES (?, ?, ?, ?);

-- name: remove_from_crew_vault!
DELETE FROM crew_vault WHERE id = ?;

-- name: award_from_crew_vault!
UPDATE crew_vault SET crew_id = NULL WHERE id = ?;
...
JOIN item_rarities ir ON i.rarity_id = ir.id
JOIN equipment_slots es ON i.slot_id = es.id
WHERE i.slot_id = ? AND i.level_requirement <= ?
...
UPDATE characters SET total_power = ? WHERE id = ?;